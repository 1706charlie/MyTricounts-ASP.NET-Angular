...
public class TricountController : AppControllerBase
{
    ...
    public async Task<ActionResult<bool>> CheckTricountTitleAvailable(TricountTitleAvailabilityDTO dto)
    {
        ...
        // Recherche d'un tricount portant déjà ce titre
        var tricount = await context.Tricounts.SingleOrDefaultAsync(t =>
            t.Title != null && t.Title.Trim().ToLower() == title);

        // CAS 1 : titre non utilisé -> disponible
        if (tricount == null)
            return Ok(true);

        // CAS 2 : titre utilisé mais par le tricount courant -> autorisé
        var tricountId = dto.TricountId;
        if (tricountId != 0 && tricount.Id == tricountId)
            return Ok(true);

        // CAS 3 : titre pris par un autre tricount
        return Ok(false);
    }
