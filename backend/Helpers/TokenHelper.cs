...
// NOTE: adapte bien ces namespaces à ta solution
using TricountApp.Models;                 // TricountContext
using TricountApp.Models.Entities;        // Role
...
public class TokenHelper
{
    ...
    */
    ...
    public static string GenerateJwtToken(IEnumerable<Claim> claims)
    {
        ...
        // NOTE tuto : clé en dur (on migrera vers appsettings plus tard)
    ...
    // V15: refresh tokens (à activer plus tard)
    /*
    public async Task<string?> GetRefreshTokenAsync(string identity)
    {
        // Exemple quand tu auras un champ RefreshToken sur User
        var user = await _context.Users.SingleOrDefaultAsync(u => u.Email == identity);
        return user?.RefreshToken;
    }

    public async Task SaveRefreshTokenAsync(string identity, string token)
    {
        var user = await _context.Users.SingleOrDefaultAsync(u => u.Email == identity);
        if (user != null)
        {
